...
def crop_image(image, label, crop_size, is_train=True):
    ...
    #label crop
    label_crop = 255*np.ones(input_shape, dtype=np.long)
    label_crop[0: h_e, 0: w_e] = label[y1: y1+ h_e, x1: x1+ w_e]
    return im, label_crop
...
class TrainDataset(torchdata.Dataset):
    def __init__(self, odgt, opt, max_sample=-1, batch_per_gpu=1):
        ...
        self.img_height, self.img_width=opt.data_shape
        self.input_size=opt.input_size
        rounded_height = round2nearest_multiple(self.input_size[0], self.segm_downsampling_rate)
        rounded_width  = round2nearest_multiple(self.input_size[1], self.segm_downsampling_rate)
        self.input_size = [rounded_height, rounded_width]
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_per_gpu):
            ...
            segm = cv2.resize(segm, (scale_w, scale_h), interpolation=cv2.INTER_NEAREST)
            #cv2.imwrite('{}_img.png'.format(self.cur_idx),img)
            #cv2.imwrite('{}_label.png'.format(self.cur_idx),segm)
