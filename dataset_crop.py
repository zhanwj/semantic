...
def crop_image(image, label, crop_size, is_train=True):
    def crop_index(ndim, new_dim, is_train):
        ...
        return sh,sw
    input_shape = crop_size #input size of network
    ...
    #label crop
    label = 255*np.ones((input_shape))
    label[0: h_e, 0: w_e] = label[y1: y1+ h_e, x1: x1+ w_e]
    return im, label
...
class TrainDataset(torchdata.Dataset):
    def __init__(self, odgt, opt, max_sample=-1, batch_per_gpu=1):
        ...
        self.img_height, self.img_width=opt.image_shape
    ...
    def _get_sub_batch(self):
        while True:
            ...
            if len(self.batch_record_list[1])==self.batch_per_gpu:
                batch_records = self.batch_record_list[1]
                self.batch_record_list[1]=[]
                break
    ...
    def __getitem__(self, index):
        ...
        batch_images = torch.zeros(self.batch_per_gpu, 3, self.input_size[0], self.input_size[1])
        batch_segms = 255*torch.ones(self.batch_per_gpu, self.input_size[0], self.input_size[1]).long()

        for i in range(self.batch_per_gpu):
            this_record = batch_records[i]
            # load image and label
            line=this_record.split()
            image_path=line[0]
            label_path=line[2]
            image_path = os.path.join(self.root_dataset, image_path)
            segm_path = os.path.join(self.root_dataset, label_path)
            img = imread(image_path, mode='RGB')
            segm = imread(segm_path)
            cv2.imwrite('img{}.jpg'.format(self.cur_idx),img)
            assert(img.ndim == 3)
            assert(segm.ndim == 2)
            ...
            this_scale = np.random.choice(self.imgSize)
            crop_size = [int(np.round(1.0*self.input_size[0]/this_scale)), 
                    int(np.round(1,0*self.input_size[0]/this_scale))] 
            img, segm = crop_image(img, segm, crop_size)
            print ('after crop', this_scale, img.shape, segm.shape)
            scale_h = int(np.round(crop_size[0]*this_scale))
            scale_w = int(np.round(crop_size[1]*this_scale))
            print ('scale shape', this_scale, (scale_h, scale_w))
            img = imresize(img, (scale_h, scale_w), interp='bilinear')
            segm = imresize(segm, (scale_h, scale_w), interp='nearest')

            img = img.astype(np.float32)[:, :, ::-1] # RGB to BGR!!!
            img = img.transpose((2, 0, 1))
            img = self.img_transform(torch.from_numpy(img.copy()))
            if segm.shape[0] % self.segm_downsampling_rate or segm.shape[1] % self.segm_downsampling_rate:
                assert False, 'input size must be down by segm_downsampling_rate'
                ...
                segm_rounded = 255*np.ones((segm_rounded_height, segm_rounded_width), dtype='uint8')
                segm_rounded[:segm.shape[0], :segm.shape[1]] = segm
                assert np.all(segm_rounded!= -1), 'segm label error'
                img_rounded=np.zeros((3, segm_rounded_height, segm_rounded_width), dtype='float32')
                img_rounded[:, :segm.shape[0], :segm.shape[1]] = img
                batch_images[i] = img_rounded
                batch_segms[i] = torch.from_numpy(segm_rounded.astype(np.int)).long()
            else:
                assert np.all(segm!= -1), 'segm label error'
                batch_images[i] = img
...
class TrainDataset_ADE20K(torchdata.Dataset):
    def __init__(self, odgt, opt, max_sample=-1, batch_per_gpu=1):
        self.root_dataset = opt.root_dataset
        self.imgSize = opt.imgSize
        self.imgMaxSize = opt.imgMaxSize
